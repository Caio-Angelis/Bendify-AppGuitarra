...
const useElectron = ['dev:electron', 'build:electron'].includes(process.env.npm_lifecycle_event ?? '');
const productionCspMetaTag = `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; ` +
    ...
    plugins: [
        react(),
        {
            ...
            transformIndexHtml(html, context) {
                ...
                return html.replace('<!-- CSP de produção é injetada no build via vite.config.ts -->', productionCspMetaTag);
            },
        },
        ...(useElectron
            ? [
